...
// level : numéro du niveau atteint
// xpRequired : XP total cumulé pour atteindre ce niveau
// reward : récompense accordée à l'atteinte du niveau
export const LEVEL_TABLE = [
  ...
  { level: 4,  xpRequired: 500,  reward: { coins: 100,  label: '+100🪙',         potSlots: 1 } },
...
// ── XP gagnée au harvest selon rareté × qualité ───────────────────────────
const XP_BY_RARITY   = { common: 10, rare: 20, epic: 40, legendary: 80, mythic: 200 };
const XP_QUALITY_MUL = [0.5, 1.0, 1.5, 2.0, 3.0]; // guezmer → comète
...
export function resolveLevel(totalXp) {
  ...
  const nextEntry       = LEVEL_TABLE[level]; // index = level car LEVEL_TABLE[0].level = 1
...
// ── Mise à jour en base ───────────────────────────────────────────────────
/**
 * Ajoute de l'XP au joueur, calcule les level-ups, crédite les récompenses.
 * @returns {{ newXp, newLevel, newCoins, leveledUp, reward }}
 */
export async function addXpToPlayer(userId, xpGained, currentData) {
  ...
  if (leveledUp) {
    // Cumule les récompenses de tous les niveaux traversés
    for (let lvl = prevLevel + 1; lvl <= newLevel; lvl++) {
      const entry = LEVEL_TABLE.find(e => e.level === lvl);
      if (entry?.reward) {
        bonusCoins += entry.reward.coins;
        reward = entry.reward; // On garde la dernière pour l'affichage
      }
    }
  }
  ...
  await supabase
    ...
    .upsert(
      { user_id: userId, xp: newTotal, level: newLevel, coins: newCoins },
      { onConflict: 'user_id' }
    );

  return { newXp: newTotal, newLevel, newCoins, leveledUp, reward };
}
