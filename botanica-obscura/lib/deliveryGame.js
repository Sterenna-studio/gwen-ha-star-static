...
function buildResultScreen(won, coinsEarned, delivered) {
  ...
  panel.innerHTML = `
    <div class="dg-result ${won ? 'dg-win' : 'dg-lose'}">
      ...
      ${won
        ? `<div class="dg-result-body">+🪙 <strong>${coinsEarned}</strong> pièces<br>🌸 ${delivered} fleurs livrées</div>`
...
function stopGame(won) {
  ...
  removeListeners();
  const coinsEarned = won ? Math.round(state.cargo * REWARD_PER_UNIT * (1 + state.wantedLevel * 0.15)) : 0;
...
/**
 * Lance le mini-jeu.
 * @param {number}   cargo       — nb de FLEURS en inventaire
 * @param {function} onComplete  — callback(coinsEarned, flowersDelivered)
...
export function launchDeliveryGame(cargo, onComplete, onFail) {
  ...
  Object.assign(state, {
    ...
    distance: BASE_DISTANCE + cargo * 30,
